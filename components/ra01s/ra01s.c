...
void SetSleep(bool retainConfig)
{
  SetStandby(SX126X_STANDBY_RC);
  ...
  WriteCommandSleep(SX126X_CMD_SET_SLEEP, &sleepMode, 1);
...
void WriteCommandSleep(uint8_t cmd, uint8_t *data, uint8_t numBytes)
{
  uint8_t status;
  for (int retry = 1; retry < 10; retry++)
  {
    status = WriteCommandSleep2(cmd, data, numBytes);
    ESP_LOGD(TAG, "status=%02x", status);
    if (status == 0)
      break;
    ESP_LOGW(TAG, "WriteCommand2 status=%02x retry=%d", status, retry);
...
uint8_t WriteCommandSleep2(uint8_t cmd, uint8_t *data, uint8_t numBytes)
{
  // ensure BUSY is low (state meachine ready)
  WaitForIdle(BUSY_WAIT);
